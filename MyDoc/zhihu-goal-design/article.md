...
这篇不是再做一个待办列表。DeepSeek Harness 用「盯住一个目标连跑几天」当探针：新能力能不能接插件、不拆发动机。接得上，它才是运行时；接不上，它只是套了工具的聊天循环。

贯穿始终的一句话是：**状态可以回放，权限必须重授。** 这不是文风。这是在验证一套运行时到底能不能当 Harness 用。
...
你想让 Agent 盯住一件事，跨很多回合，直到真正做完。第一反应通常是三件快事：改主循环、加一个计数器、在提示词里写「请继续完成目标」。这三条都会在同一处裂开：重启后没人说得清当时叮嘱了什么；拷出来的会话自己跑起来；模型把题目偷偷改小。
