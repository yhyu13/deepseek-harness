...
1. **别把目标焊进主循环。** 不是在 loop 里加分支。
2. **状态可回放，权限必须重授。** 不是恢复后自己接着跑。
3. **模型看见的，必须能翻出来。** 内存叮嘱不算目标功能。
4. **同时改一份账，过期的丢掉。** 谁后写谁赢是 bug。
5. **「我做完了」不算完。** 听工作区。卡住要连续出现，才允许认输。
6. **记账法可以不同，真相只能一本。** 一行表或一串事件，二选一。
7. **树根和主干错了，叶子再漂亮也会在同一处裂开。** 改 loop、把「准续」写进日志、用提示词当权限，会一起爆。

**结论**

给 Agent 加长期目标，不是让它「记得继续」。是让世界有一本可回放的账，并且每一次真正跑起来，都重新拿到执行授权。能抄走的是这套切割。

**PS.** 只带走一句的话：状态可回放，权限必须重授。

**PPS.** 这和「把模型训得更听话」不是一回事。提示词管该不该；校验管能不能。

**PPPS.** 文里的名字都会过时。你自己的系统里如果只改 loop、把「准不准续」写进日志、用提示词当权限——裂开的会是同一刀。
